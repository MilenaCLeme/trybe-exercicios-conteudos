...
const getPlanet = () => {
  ...
  setTimeout(() => console.log("Returned planet: Marte =", mars.name), 4000);
...
function getPokemonDetails(filter, callback) {
  setTimeout(() => {
    if (pokemons.find((filtar) => filtar.name = filter) === undefined) {
      return callback(new Error('Não temos esse pokémon para você :('), null);
    }
    const pokemon = pokemons.find((filtar) => filtar.name = filter);
